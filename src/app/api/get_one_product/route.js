      ...
      // pour les caracteristique variante
        const characteristics = await CaracteristiqueModal.find({
          ...
          // pour les caracteristique couleur
            ...
            const variantCombinationsRaw = await productVariantsCollection
              .find({ productId: product._id })
              .toArray();

            const variantCombinations = variantCombinationsRaw.map((combo) => ({
              _id: combo._id,
              options: combo.options || [],
              isActive: combo.isActive !== false,
              priceAdjustment: combo.priceAdjustment || 0,
              stock: combo.stock || 0,
              sku: combo.sku || "",
            }));
