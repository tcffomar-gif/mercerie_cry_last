...
export function calculateTotalPrice(reductions, totalQuantity, unitPrice) {
  const normalizedReductions = Array.isArray(reductions) ? reductions : [];

  // Trier les réductions par quantité décroissante
  normalizedReductions.sort((a, b) => b.quantite - a.quantite);
  ...
  for (const reduction of normalizedReductions) {
...
export function calculateTotalPricev2(reductions, totalQuantity, totalPrice) {
  ...
  const sortedReductions = [...normalizedReductions].sort(
  ...
  for (const reduction of sortedReductions) {
    ...
    const reductionQuantity = Number(reduction?.quantite || 0);
    const reductionAmount = Number(reduction?.reduction || 0);

    if (reductionQuantity <= 0 || reductionAmount <= 0) continue;
